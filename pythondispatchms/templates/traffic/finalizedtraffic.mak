...
<script>
function finalizedTraffic(){
    var bell = new Audio("${tg.url('/sounds/ding.mp3')}");
    var icc='';

    function finalizedDateFmatter ( cellvalue, options, rowObject )
    {
    ...
    function finalizedPriorityFmatter ( cellvalue, options, rowObject )
        {
            html=cellvalue;
    ...
    function finalizedYellowFmatter ( cellvalue, options, rowObject )
        {
            html = '<center><div style="background-color:yellow" style="text-align:center">'+cellvalue+'</div></center>';
            return html;
        }
    function finalizedStatusFmatter ( cellvalue, options, rowObject ){
        html=cellvalue;
        if ( cellvalue=="C"){html = '<center><span class="glyphicon glyphicon-copyright-mark" style="color:blue"></span></center>';}
    ...
    var c5Plates=''
    var grid_name = '#jqGridFinalized';
    var grid_pager= '#listPagerAlertsf';
    var update_url='${h.url()}/traffic/updateTraffic?group=${group}';
    var load_url  ='${h.url()}/traffic/loadfinalizedTraffic?group=${group}';
    ...
    var grid = jQuery(grid_name);
            ...
            $(document).ready(function () {
                ...
                function finalizedMake_base_auth(user, password) {
                ...
                $.ajaxSetup({
                    ...
                    xhr.setRequestHeader( "Authorization", finalizedMake_base_auth(username,password));
                    }
                });
                grid.jqGrid({
                url: load_url,
                ...
                colModel: [
                    ...
                    {name: 'attended_state', index: 'attended_state', width: 5,align: 'right',hidden: false, editable: true, edittype: 'text', formatter:finalizedStatusFmatter, editrules: {required: true},search:false},
                    {name: 'priority', index: 'priority', width: 7,align: 'right',hidden: false, editable: true, edittype: 'text', formatter:finalizedPriorityFmatter, editrules: {required: true},search:false},
                    ...
                    {name: 'time_stamp',index: 'time_stamp', width: 14, align: 'right',hidden: false,editable: true, edittype: 'text', formatter:finalizedDateFmatter,editrules: {required: false}},
            ...
            function finalizedDoubleClick(rowId){
                var rowData = jQuery('#jqGridFinalized').getRowData(rowId);
                ...
                function finalizedUpdateState(rowId,state){
                    $.ajax({
                        type: "GET",
                        ...
                        success: function(data) {
                            ...
                            $('#jqGridFinalized').trigger( 'reloadGrid' );
                            finalizedActivateTab('eventData');
                            $('#commentFormf')[0].reset();
                            return true;
                        },
                        error: function() {
                ...
                function PoP() {
                                ...
                                var putpendigDialog = $( "#putpendingFormf" ).dialog({
                                        autoOpen: false,
                                        ...
                                        buttons: {
                                            Submit: function() { //submit
                                                ...
                                                $( "#pendingFormf" ).validate({
                                                                  rules: {
                                                                       pendingcomment:{required:true, wordCountPending: ['10']},
                                                                      }
                                                                });
                                                if($("#pendingFormf").valid()){   // test for validity
                                                  var row = $(this).data('rowId');
                                                  finalizedUpdateState(row,"P");
                                                  $( this ).dialog( "close" );
                ...
                function finalizedActivateTab(tab){
                    $('.nav-tabs a[href="#' + tab + '"]').tab('show');
                };
                function finalizedInitMap(latitude,longitude,cell_lat,cell_lon){
                    var myLatLng = {lat: latitude, lng: longitude};
                   // var myLatLng = new google.maps.LatLng(latitude, longitude);
                    var mapfinalize = new google.maps.Map(document.getElementById("map_canvas_fin"), {
                      zoom: 12,
                      center: myLatLng
                    });
                    ...
                    google.maps.event.trigger(mapfinalize, 'resize');
                    $("#map_canvas_fin").css("width", Math.round(window.innerHeight*.40)).css("height", Math.round(window.innerHeight*.40));

                    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
                      var target = $(e.target).attr("href") // activated tab

                        if (target==='#locationMapf'){
                            google.maps.event.trigger(mapfinalize, 'resize');
                            mapfinalize.setCenter(marker.getPosition());
                        }

                    });
                }

                var trafficDialog = $( "#finalizedtrafficForm" ).dialog({
                        autoOpen: false,
                        height: winHeight,
                        width: winWidth,
                        modal: true,
                ...
                var submitFinalizeButtons = {
                            ...
                            //add the custom validation method
                            $( "#commentFormf" ).validate({
                                              rules: {
                                                   comment:{required:true, wordCount: ['10']},
                                                  }
                                            });
                            if($("#commentFormf").valid()){   // test for validity
                             var row = $(this).data('rowId');
                             finalizedUpdateState(row,'C');
                             finalizeDialog.dialog( "close" );
                             trafficDialog.dialog( "close" );
                                }
                            },
                ...
                var finalizeDialog = $( "#finalizeFormf" ).dialog({
                        autoOpen: false,
                        height: winHeight-100,
                        width: winWidth-200,
                        modal: true,
                ...
                $("#helpCommentf").change(function(){
                  var id = $(this).find("option:selected").attr("value");
                  $("#comment").val(id);
                });
                $("#helpComment2f").change(function(){
                  var id = $(this).find("option:selected").attr("value");
                ...
                url: '${h.url()}/traffic/getRecord?record='+rowId+"&user=${user}"+"&group=${group}"+"&state=closed"+"&tz=${tz}",
                contentType: "application/json; charset=utf-8",
                data: { 'param':'gaugeParameters' },
                success: function(parameterdata) {
                    ...
                    $("#accountf").html(parameterdata.account);
                    $("#eventf").html(parameterdata.event);
                    $("#vehiclef").html(parameterdata.vehicle);
                    $("#callerListf").html(parameterdata.callerlist);
                    $("#accountHistoryf").html(parameterdata.accounthistory);
                    $("#comment").val(parameterdata.comment);
                    $("#alarmcheck").val(parameterdata.false_alarm);
                    $("#eventLink").html(parameterdata.platform);
                    $("#imagesf").html(parameterdata.images);
                    // Render time in local
                    $('#ts').text(finalizedDateFmatter(parameterdata.ts));
                    $('#et').text(finalizedDateFmatter(parameterdata.et));
                    $('#pendingHistf').html(parameterdata.pendinghistory);
                    $('#finalizedHistf').html('<br><br><p>'+parameterdata.comment+'</p>');
                    $('#jqGridFinalized').trigger( 'reloadGrid' );
                    $("#finalizedtrafficForm").show();
                    $("#dialog").dialog();
                    c5plates=parameterdata.plates;
                    urlinstall=parameterdata.link;
                    icc=parameterdata.icc;
                    finalizedInitMap(parameterdata.latitude, parameterdata.longitude,parameterdata.cell_lat,parameterdata.cell_lon);
                    finalizedActivateTab('accountf');
...
$( document ).ready(function() {
    ...
    <!-- page start-->
    <div id="finalizedfinalizedtrafficForm01"  class="dialog-hidden" title="${_('Traffic')}">
    </div>
    <div id="finalizedtrafficGrid01" class="dialog-hidden" title="${_('Evento por llamada')}">
        <table style="width:100%;overflow:auto;">
        <table id="finalizedjqGrid" class="scroll" cellpadding="0" cellspacing="0"></table>
        <div id="finalizedPagerTables" class="scroll" style="text-align:center;"></div>
    ...
    <!-- Hidden POP UP start-->
     <div id="finalizedAddImageForm" class="dialog-hidden" title="${_('Upload Image')}">
        <form action="${h.url()}/traffic/uploadImage" name= "finalizedImagesForm" id="finalizedImagesForm" method="POST" enctype="multipart/form-data">
    ...
    <div id="finalizedtrafficForm" class="dialog-hidden" title="${_('Traffic')}">
        <ul class="nav nav-tabs">
          <li><a data-toggle="tab" href="#accountf" >${_('Account')}</a></li>
          <li><a data-toggle="tab" href="#eventf" >${_('Event')}</a></li>
          <li><a data-toggle="tab" href="#vehiclef" >${_('Vehicle')}</a></li>
          <li><a data-toggle="tab" href="#callerListf" >${_('Caller List')}</a></li>
          <li><a data-toggle="tab" href="#accountHistoryf">${_('Account History')}</a></li>
          <li><a data-toggle="tab" href="#imagesf">${_('Images')}</a></li>
          <li><a data-toggle="tab" href="#installationReportf">${_('Installation Report')}</a></li>
          <li><a data-toggle="tab" href="#pendingHistf">${_('Historial')}</a></li>
          <li><a data-toggle="tab" href="#finalizedHistf">${_('Finalizado')}</a></li>
          <li><a data-toggle="tab" href="#locationMapf" data-toggle="tab">${_('Map')}</a></li>
        </ul>

        <div class="tab-content">
          <div id="accountf" class="tab-pane fade in active">
          </div>
          <div id="eventf" class="tab-pane fade in active">
          </div>
          <div id="vehiclef" class="tab-pane fade in active">
          </div>
          <div id="callerListf" class="tab-pane fade">
          </div>
          <div id="accountHistoryf" class="tab-pane fade">
          </div>
          <div id="imagesf" class="tab-pane fade">
          </div>
          <div id="installationReportf" class="tab-pane fade">
              <br>
              <button id="button" type="button" class="btn btn-default" data-toggle="button">${_('Print')}</button>
          </div>
          <div id="pendingHistf" class="tab-pane fade">
              <br>
              <p>No hay eventos</p>
          </div>
          <div id="finalizedHistf" class="tab-pane fade">
              <br>
              <p>No hay Eventos</p>
          </div>
          <div id="locationMapf" class="tab-pane fade">
    ...
    <div id="finalizeFormf" class="dialog-hidden" title="${_('Finalize')}">
      <form id="commentFormf">
        <fieldset>
            <select id="helpCommentf" name="helpComment">
                % for item in list:
    ...
    <div id="putpendingFormf" class="dialog-hidden" title="${_('Pending Comment')}">
      <form id="pendingFormf">
        <fieldset>
            <select id="helpComment2f" name="helpComment">
                % for item in list:
                    <option id="${item['id2']}" value="${item['hcomment']}">${item['hcomment']}</option>
                % endfor
            </select>
            <br>
    ...
    <div id="createTicketFormf" class="dialog-hidden" title="${_('Create Ticket')}">
      <form id="ticketFormf">
        <fieldset>
    ...
    <div id="createC5Formf" class="dialog-hidden" title="${_('Datos para enviar al C5')}">
        <h3>Campos:</h3>
...
<div id="noteContent" title="${_('Basic dialog')}">
    ...
    <table id="jqGridFinalized" class="scroll" cellpadding="0" cellspacing="0"></table>
    <div id="listPagerAlertsf" class="scroll" style="text-align:center;"></div>
